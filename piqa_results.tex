...
\begin{table}[h]
    ...
    \renewcommand{\arraystretch}{1.12}
    \caption{
        Membership inference results on PIQA. AUC is computed on each duplication level's members against all dup~$=0$ non-members. \textbf{Projection} is $-g(z)^\top\Delta\theta$, a first-order log-likelihood ratio between the perturbed model and the suppressed model. Reference is an \textbf{oracle} method using the same ratio but with the \textit{standard} rather than suppressed model. The \textbf{All} row reports the unweighted mean across duplication levels.
    }
    \vspace{2pt}
